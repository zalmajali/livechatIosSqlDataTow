...
export class ChatbotPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.sessionLogin = await this.storage.get('sessionLogin');
      ...
      this.startPolling();
      let showLoading= await this.storage.get('showLoadingCaht');
      if(showLoading == 0){
        await this.storage.set('showLoadingCaht','1');
...
ngOnDestroy() {
  ...
  async functionReturnData(){
      const processChats = this.conversations.filter(c => c.type == 3);
  ...
  functionContact(){
    this.navCtrl.navigateRoot(['/contacts', {backUrl:3}]);
  }
  functionHome(){
  ...
  functionChats(number:any,chatSessionId:any,userName:any,name:any){
    this.navCtrl.navigateRoot(['/chats', {number:number,chatSessionId:chatSessionId,userNameUsed:userName,backUrl:2,name:name}]);
  }
